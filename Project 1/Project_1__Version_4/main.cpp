    ...
    cout << "Dice roll Sum is : "<<diceSum<<endl;
        ...
        pOptn = toupper(pOptn); //Makes it so a lowercase answer is acceptable
            ...
            if(pOptn == 'Y'){ //To play Pass Line rules
                ...
                if(diceSum == 7 || diceSum == 11){ //7 or 11 roll is a win
                    cout << "You doubled your money!"<<endl;
                }
                else if(diceSum == 2 || diceSum == 3 || diceSum == 12){ //2,3, or 12 loses
                    cout << "You lost..."<<endl;
                }
                else{ //Any number besides 7,11,2,3, or 12 activates the point rules
                    cout << "You got a point: "<<diceSum<<endl;
                    cout << "Try to roll "<<diceSum<< " again before a 7 to double your money!"<<endl;
                    point = 0;
                    while(point < 1){ //While loop to insure that the dice can only roll a 7 or the previous roll
                        dceOneP = rand() % num + 1;
                        dceTwoP = rand() % num + 1;
